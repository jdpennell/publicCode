...
/** You'll need to make a MainActivity.mainActivity reference handy */
...
public class ImageManipulator
{
    ...
    public static Bitmap circleCrop(@Nullable Bitmap source)
    {
        ...
        if (null != source)
        {
            ...
            copy = Bitmap.createScaledBitmap(source, mindim, mindim, doBilinearInterpolation);
        }
    ...
    /**
     * Returns a copy of source that has been cropped so the corners are filleted
     *
    ...
    public static Bitmap filletCorners(@Nullable Bitmap source, float radius)
    {
        ...
        if (null != source)
        {
            copy = Bitmap.createBitmap(source.getWidth(), source.getHeight(), storageScheme);
        }

        // paint from the source onto a new blank canvas, then return the newly created image
        if (null != copy)
        {
            ...
            rect = new RectF(0.0f, 0.0f, (float) source.getWidth(), (float) source.getHeight());
    ...
    /**
     * Returns a copy of source that has been scaled to the phone or tablet's screen
     *
    ...
    public static Bitmap scaleToScreen(@Nullable Bitmap source)
    {
        ...
        {
            out = Bitmap.createScaledBitmap(source, params.width, params.height, doBilinearInterpolation);
        }
     ...
     *
     * @param degree (optional) a number between 0 and 1 which determines the amount of
     *               bluring to perform. Higher numbers for more blur.
     *               Any value higher than 1 is treated as 1. Default : 0.75
    ...
    public static Bitmap blur(@Nullable Bitmap source, double degree)
    {
        ...
        if (null != source) {
            ...
            copy = Bitmap.createScaledBitmap(source, scaledWidth, scaledHeight, doBilinearInterpolation);

            // ...then let's grow the image back to full size...
            if (null != copy) {
                copy = Bitmap.createScaledBitmap(copy, source.getWidth(), source.getHeight(), doBilinearInterpolation);
            }
    ...
    protected static Resources getResources()
    {
        ...
        {
            out = MainActivity.mainActivity.getResources();
        }
    ...
    /** Minimum size when scaling */
    protected static final int MIN_SCALE_DIM = 10;
}
